var GridConfigObject = function (ContextArray) {
    ...
    Grid.LocalAttributes = {
        ...
        DefaultAltCellBackColor: '#eeeeee',
        DefaultAltCellFontColor: 'gray',
    ...
    Grid.BuildGridControl = function (Collection) {

        //$("#GridList").html("");
        var ListContainer = Grid.GetListContainer();
        if (!(Collection.length == 0 || Collection == null)) {

            if (Grid.GridAttributes.GridColumnArrange.length != 0) Collection = Grid.ArrangeColumns(Collection);
                ...
                for (var key in Collection[i]) {
                    ...
                    if (Grid.LocalAttributes.ExcludeColumnIdexes[ColCount] != 1) {
                        ...
                        if (i == 0 && !Grid.LocalAttributes.GridHeaderIsBuilt) {
                            ...
                            //$("#" + Grid.GridAttributes.GridListContainer + "_HeaderRow").html(Grid.LocalAttributes.GridHeader);
            ...
            //<span id = 'GridHeaderRow' /><span id = 'GridList' />
            $("#" + Grid.GridAttributes.GridListContainer + "_HeaderRow").html(Grid.LocalAttributes.GridHeader);
            $("#TableListContainer").html(ListItemConcat);

//            ListContainer = ListContainer.replace("$LIST$", ListItemConcat);
//            $("#GridHeaderRow").html(Grid.LocalAttributes.GridHeader);
            //$("#GridList").html(ListContainer);
    ...
    Grid.BuildGrid = function (Collection) {
        ...
        if (!(Collection.length == 0 || Collection == null)) {
            ...
            var GridContainer = Grid.GetGridTemplate();
            var ListContainer = Grid.GetListContainer();
            var HeaderStyle = Grid.CreateStyle("Header");
            var ContentStyle = Grid.CreateStyle("Cell");
            var ListItemConcat = "";
                ...
                for (var key in Collection[i]) {
                    ...
                    if (Grid.LocalAttributes.ExcludeColumnIdexes[ColCount] != 1) {
                        ...
                        if (i == 0 && !Grid.LocalAttributes.GridHeaderIsBuilt) {
                            ...
                            Grid.BuildHeader(ColCount, HeaderText, HeaderStyle, key);
                        ...
                        NewItem += Grid.BuildContentCell(ColCount, Collection[i][key], ContentStyle, GridRowId)
    ...
    Grid.GetGridTemplate = function () {
        ...
        var GridTemplateEnd = "$HEADER$$CONTENT$$TOOLBAR$</table>";
        //var GridTemplateEnd = "<span id = 'GridHeaderRow' /><span id = 'GridList' />$TOOLBAR$</table>";
        ...
        var HeaderCellRow = Grid.GetCellRow(Grid.GridAttributes.GridListContainer + "_HeaderRow", "", "", "Header");
        GridTemplate = GridTemplate.replace("$HEADER$", HeaderCellRow);

        var ListContainer = Grid.GetListContainer();
        GridTemplate = GridTemplate.replace("$CONTENT$", HeaderCellRow);
    ...
    Grid.GetListContainer = function () {
        ...
        var ListContainerEnd = "</table></div></td></tr>";
    ...
    Grid.CreateStyle = function (Type) {
        ...
        var style = 'text-align: $ALIGN$; $BORDER$ width: $WIDTH$PX; padding-top: $HEIGHT$px; padding-bottom: $HEIGHT$px; padding-right: $PADDING$px; padding-left: $PADDING$px;'
    ...
    Grid.SetCurrentSelectedRow = function (SelectedRow) {
        ...
        if (Grid.LocalAttributes.GridCurrentSelectedRowId != null) {
            ...
            $("#" + Grid.LocalAttributes.GridCurrentSelectedRowId).css({ 'background-color': Grid.LocalAttributes.GridCurrentSelectedRowBG, 'color': Grid.LocalAttributes.GridCurrentSelectedRowFG });
        ...
        SelectedRow.css({ 'background': '#ffffaa', 'color': 'blue' });
    ...
    Grid.TakeActionOnRow = function (SelectedRow) {
        ...
        Grid.FireEvent("OnGridItemSelected", null);
    ...
    Grid.GetSortField = function () {
        ...
        if (SortField == null) SortField = Grid.GridAttributes.GridListIdField;
        return SortField;
